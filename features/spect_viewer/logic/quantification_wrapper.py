...
def get_quantification_input_paths_v2(patient_folder: Path, filename_stem: str) -> dict:
    ...
    logging.info(f"🔧 [DEBUG WRAPPER] Getting quantification input paths:")
    logging.info(f"🔧 [DEBUG WRAPPER]   Patient folder: {patient_folder}")
    logging.info(f"🔧 [DEBUG WRAPPER]   Filename stem: {filename_stem}")
    logging.info(f"🔧 [DEBUG WRAPPER]   Folder exists: {patient_folder.exists()}")
    ...
    try:
        ...
        logging.info(f"🔧 [DEBUG WRAPPER] Classification files:")
        logging.info(f"🔧 [DEBUG WRAPPER]   Anterior mask: {ant_clf_files['mask_original'].exists()}")
        logging.info(f"🔧 [DEBUG WRAPPER]   Posterior mask: {post_clf_files['mask_original'].exists()}")
        ...
        logging.info(f"🔧 [DEBUG WRAPPER] Segmentation files:")
        logging.info(f"🔧 [DEBUG WRAPPER]   Anterior seg: {ant_seg_files['png_colored'].exists()}")
        logging.info(f"🔧 [DEBUG WRAPPER]   Posterior seg: {post_seg_files['png_colored'].exists()}")
        ...
        logging.info(f"🔧 [DEBUG WRAPPER] Final paths:")
        for key, value in paths.items():
            exists = value.exists() if hasattr(value, 'exists') else False
            logging.info(f"🔧 [DEBUG WRAPPER]   {key}: {value} (exists: {exists})")
        
        return paths
        
    except Exception as e:
        logging.info(f"🔧 [DEBUG WRAPPER]  Error getting paths: {e}")
        import traceback
        traceback.print_exc()
        
        # Fallback to old method
        logging.info(f"🔧 [DEBUG WRAPPER] Using fallback method...")
        return {
...
def run_quantification_for_patient_v2(dicom_path: Path, patient_id: str, study_date: str) -> bool:
    ...
    try:
        ...
        logging.info(f"     Starting V1.2 BSI quantification for patient {patient_id}")
        logging.info(f"     New workflow: Color-based segmentation + Single/Dual view processing")
        ...
        if not can_proceed:
            logging.info(f"     Cannot proceed with V1.2 quantification. Missing files: {', '.join(missing_files)}")
            logging.info(f"     Need at least ONE complete pair (anterior OR posterior segmentation + classification)")
            return False
        
        #   NEW: Log what we have available
        logging.info(f"     File availability check:")
        logging.info(f"       Anterior pair available: {has_anterior_pair}")
        logging.info(f"       Posterior pair available: {has_posterior_pair}")
        
        if missing_files:
            logging.info(f"     [INFO] Some files missing (but proceeding): {', '.join(missing_files)}")
        
        logging.info(f"     Processing V1.2 quantification with color-based segmentation...")
        ...
        if has_anterior_pair:
            logging.info(f"     Processing anterior view...")
            try:
                anterior_results = calculate_pixel_bsi_v2(
                    str(available_paths['segmentation_anterior']),
                    str(available_paths['hotspot_anterior'])
                )
                logging.info(f"       Anterior processing completed")
            except Exception as e:
                logging.info(f"      Anterior processing failed: {e}")
                anterior_results = None
        else:
            logging.info(f"     ⏭️ Skipping anterior view (files not available)")
        
        # Process posterior view if available
        if has_posterior_pair:
            logging.info(f"     Processing posterior view...")
            try:
                posterior_results = calculate_pixel_bsi_v2(
                    str(available_paths['segmentation_posterior']),
                    str(available_paths['hotspot_posterior'])
                )
                logging.info(f"       Posterior processing completed")
            except Exception as e:
                logging.info(f"      Posterior processing failed: {e}")
                posterior_results = None
        else:
            logging.info(f"     ⏭️ Skipping posterior view (files not available)")
        
        #   Check if we got at least one result
        if not anterior_results and not posterior_results:
            logging.info(f"      No views processed successfully")
            return False
        
        #   NEW: Calculate combined BSI based on available data
        logging.info(f"     Calculating BSI scores...")
        ...
        if anterior_results and posterior_results:
            logging.info(f"     Scenario: Both views available")
            ...
            logging.info(f"       Dual-view BSI quantification completed")
            logging.info(f"     Anterior BSI: {combined_results['anterior_bsi']:.3f}")
            logging.info(f"     Posterior BSI: {combined_results['posterior_bsi']:.3f}")
            logging.info(f"     Combined BSI: {combined_results['combined_bsi']:.3f}")
            
        # Scenario 2: Only anterior available
        elif anterior_results and not posterior_results:
            logging.info(f"     Scenario: Only anterior view available")
            ...
            logging.info(f"       Single-view (anterior) BSI quantification completed")
            logging.info(f"     Anterior BSI: {anterior_bsi:.3f}")
            logging.info(f"     Note: Posterior files not available")
            
        # Scenario 3: Only posterior available
        elif posterior_results and not anterior_results:
            logging.info(f"     Scenario: Only posterior view available")
            ...
            logging.info(f"       Single-view (posterior) BSI quantification completed")
            logging.info(f"     Posterior BSI: {posterior_bsi:.3f}")
            logging.info(f"     Note: Anterior files not available")
        
        logging.info(f"     Results saved to patient folder")
        return True
        
    except Exception as e:
        logging.info(f"     V1.2 Quantification error for patient {patient_id}: {e}")
        import traceback
