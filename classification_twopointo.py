...
def create_xml_annotation(image_path, results, output_xml_path):
    ...
    database.text = 'Generated from classification results'
    ...
    for result in results:
        ...
        xmin.text = str(result['bounding_box']['xmin'])
        ymin = ET.SubElement(bndbox, 'ymin')
        ymin.text = str(result['bounding_box']['ymin'])
        xmax = ET.SubElement(bndbox, 'xmax')
        xmax.text = str(result['bounding_box']['xmax'])
        ymax = ET.SubElement(bndbox, 'ymax')
        ymax.text = str(result['bounding_box']['ymax'])
    
    tree = ET.ElementTree(annotation)
    tree.write(output_xml_path, encoding='utf-8', xml_declaration=True)

def inference_classification(path_raw, path_segment, path_hotspot, path_xml, output_dir=None):
    ...
    for result in results:
        output_dict = {
            ...
            'prediction': result['prediction'],  # Note: Typo in your output
            ...
            'raw_features': {k: v for k, v in result.items()
                            if k not in ['xmin', 'ymin', 'xmax', 'ymax',
                                        'segment', 'label', 'prediction',  # Note: Typo
    ...
    if output_dir:
        xml_filename = os.path.splitext(os.path.basename(path_raw))[0] + '.xml'
        xml_path = os.path.join(output_dir, xml_filename)
        create_xml_annotation(path_raw, output_list, xml_path)
