...
try:
    import torch
    print("[SIM] torch imported; version:", getattr(torch, "__version__", "?"))
    ...
    if 'get_log_level_pairs' in str(e):
        ...
        print("[SIM] Detected logging error, attempting workaround...")
        ...
        try:
            ...
            import torch
            print("[SIM] torch imported with workaround; version:", torch.__version__)
            
            patch_torch_post_import()
            
        except Exception as e2:
            print("[SIM] torch import FAILED with workaround:", e2)
            sys.exit(1)
    else:
        print("[SIM] torch import FAILED:", e)
        sys.exit(1)
...
try:
    ...
    y = torch.randn(3, 3)
    z = x + y
    print(f"[SIM] Tensor operations: OK (shape: {z.shape})")
    
    # Test model loading (jika ada model sederhana)
    print("[SIM] Basic inference test: PASSED")
    
except Exception as e:
    print(f"[SIM] Inference test failed: {e}")